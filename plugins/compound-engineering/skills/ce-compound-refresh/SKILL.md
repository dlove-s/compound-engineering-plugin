...
- **Auto memory** (Claude Code only) — does the auto memory directory contain notes in the same problem domain? Read `${CLAUDE_PROJECT_DIR}/memory/MEMORY.md`. If the file does not exist or is empty, skip this dimension. A memory note describing a different approach than what the learning recommends is a supplementary drift signal.
...
>
> Also read `${CLAUDE_PROJECT_DIR}/memory/MEMORY.md` if it exists (Claude Code only). Check for notes related to the learning's problem domain. Report any memory-sourced drift signals separately from codebase-sourced evidence, tagged with "(auto memory [claude])" in the evidence section. If the file does not exist or is empty, skip this check.
